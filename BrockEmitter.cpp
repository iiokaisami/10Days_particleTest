...
void BrockEmitter::LaserDraw()
{
	for (int i = 0; i < 2; i++)
	{
		Novice::DrawQuad((int)laser_[i].LT.x, (int)laser_[i].LT.y,
			(int)laser_[i].RT.x, (int)laser_[i].RT.y,
			(int)laser_[i].LB.x, (int)laser_[i].LB.y,
			(int)laser_[i].RB.x, (int)laser_[i].RB.y,
			0, 0, 66, 66,
